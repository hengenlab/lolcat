...
def run(config, root, eval_batch_size=512, logdir=None):
    ...
    # get data
    train_dataset = NeuropixelsDGTorchDataset(root, 'train', k='3', random_seed=config['split_seed'], num_bins=config['num_bins'], transform=transform, min_presence_ratio=config['min_presence_ratio'], min_amplitude_cutoff=config['min_amplitude_cutoff'], max_isi_violations=config['max_isi_violations']).to(device)
    train_eval_dataset = NeuropixelsDGTorchDataset(root, 'train', k='3', random_seed=config['split_seed'], num_bins=config['num_bins'], transform=normalize, min_presence_ratio=config['min_presence_ratio'], min_amplitude_cutoff=config['min_amplitude_cutoff'], max_isi_violations=config['max_isi_violations']).to(device)
    val_dataset = NeuropixelsDGTorchDataset(root, 'val', k='3', random_seed=config['split_seed'], num_bins=config['num_bins'], transform=normalize, min_presence_ratio=config['min_presence_ratio'], min_amplitude_cutoff=config['min_amplitude_cutoff'], max_isi_violations=config['max_isi_violations']).to(device)
    test_dataset = NeuropixelsDGTorchDataset(root, 'test', k='3', random_seed=config['split_seed'], num_bins=config['num_bins'], transform=normalize, min_presence_ratio=config['min_presence_ratio'], min_amplitude_cutoff=config['min_amplitude_cutoff'], max_isi_violations=config['max_isi_violations']).to(device)
    nm_test_dataset = NeuropixelsNMTorchDataset(root, 'test', k='3', random_seed=config['split_seed'], num_bins=config['num_bins'], transform=normalize).to(device)
...
def train_on_split(split_id):
    ...
    logdir = './runs/np_dg_k=3_newdata_testing/run_{}'.format(split_id)
