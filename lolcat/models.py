...
class LOLCAT(nn.Module):
    ...
    def forward(self, x, batch, return_attention=False):
        ...
        if return_attention:
            return logits, {'global_emb': global_emb, 'attention': gate}
        else:
...
class LOLCATwConfidence(nn.Module):
    ...
    def forward(self, x, batch, return_attention=False):
        ...
        logvar = self.logvar(global_emb)

        if return_attention:
            return logits, logvar, global_emb, gate
        else:
            return logits, logvar, global_emb
...
class MultiHeadPooling(torch.nn.Module):
    ...
    def forward(self, x, batch, return_attention=False):
        ...
        for pool in self.pool:
            out.append(pool(x, batch, return_attention=return_attention))

        if return_attention:
            out, gate = zip(*out)
            gate = torch.cat(gate, dim=1)
