...
from lolcat import V1DGTorchDataset, CalciumNMTorchDataset
...
def run(config, root, eval_batch_size=512, logdir=None):
    ...
    # normalize
    train_dataset = V1DGTorchDataset(root, 'train', k='13', random_seed=config['split_seed'], num_bins=128)
    ...
    # get data
    train_dataset = V1DGTorchDataset(root, 'train', k='13', random_seed=config['split_seed'], num_bins=128, transform=transform).to(device)
    train_eval_dataset = V1DGTorchDataset(root, 'train', k='13', random_seed=config['split_seed'], num_bins=128, transform=normalize).to(device)
    val_dataset = V1DGTorchDataset(root, 'val', k='13', random_seed=config['split_seed'], num_bins=128, transform=normalize).to(device)
    test_dataset = V1DGTorchDataset(root, 'test', k='13', random_seed=config['split_seed'], num_bins=128, transform=normalize).to(device)
