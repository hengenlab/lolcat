...
def run(config, root, eval_batch_size=512, logdir=None):
    ...
    classifier = MLP([4 * feature_size, feature_size, num_classes], dropout=config['net_dropout'])
    ...
    # load weights
    checkpoint = torch.load('./runs/calcium_dg_13_go_up/model.pt').state_dict()
    ...
    scheduler1 = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50, 100], gamma=0.1)
    # scheduler2 = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=50, T_mult=1, eta_min=0)
    ...
    for epoch in tqdm(range(1, config['epochs'] + 1)):
        ...
        scheduler1.step()
        # scheduler2.step()
        writer.add_scalar('learing_rate', optimizer.param_groups[0]['lr'], epoch)
...
def main():
    ...
    logdir = './runs/calcium_dg_15_go_up_sgd'
